...
@Component({
  ...
  template: `
    <div class="chat-wrapper">
      <div class="chat-header">Coordinator Copilot</div>

      <div class="messages" role="log" aria-label="Conversation">
        @if (messages().length === 0) {
          <p class="empty-state">
            Ask about your panel — e.g. "Show me all overdue statin members"
          </p>
        }
...
export class CoordinatorCopilot {
  ...
  sendMessage(input: HTMLInputElement): void {
    ...
    this.agentService.chatCoordinator(message).subscribe({
      next: (response) => {
